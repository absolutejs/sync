...
- Compose application and package-owned local schemas with independent stable
  component IDs and version ledgers. IndexedDB upgrades all required component
  steps across every principal in one transaction and reports retained orphaned
  component ledgers when a pack is removed.
- Add JSON-safe `set-default`, `rename-field`, `remove-field`, and
  `delete-collection` operations. AbsoluteJS builds and service workers can carry
  these generated migrations without serializing executable functions.
- Preserve the 2.25 single-schema API as the `@absolutejs/app` component and keep
  its status shape backward-compatible. Add tests for JSON round trips,
  deterministic multi-pack composition, legacy-ledger adoption, and duplicate
  component rejection.
...
- Export `PlatformSyncHealthSchema` and its static type so hosts validate the
  managed readiness contract and release identity without redefining it.

## [2.7.0] — 2026-07-16

- The platform-managed runtime now owns first-class Sync pack registration and
  cluster-bus connection lifecycle through `packs` and `clusterBus`.
- Runtime readiness stays closed until the cluster subscription connects and
  reports the connected cluster posture plus registered packs for operators.
- `runtime.ready` and idempotent `runtime.dispose()` remove manual startup and
  shutdown wiring from hosted applications.
...
- `sync({ resolveTopics })` now accepts asynchronous authorization, allowing
  database-backed sessions to resolve their permitted SSE topics before the
  stream opens.
